"""Admin CRUD-эндпоинты для API-ключей carousel.

DUP: копия Modules/script/admin_keys.py с префиксом /carousel/admin.
Переиспользует pricing/keys из viral_llm.
"""
...
async def create_key(req: CreateKeyReq):
    if req.provider not in ALL_PROVIDERS:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=f"unknown_provider:{req.provider}")
    try:
        k = state.key_store.create(
            provider=req.provider, label=req.label, secret=req.secret,
            priority=req.priority, monthly_limit_usd=req.monthly_limit_usd,
            is_active=req.is_active,
        )
    except Exception as e:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=f"create_failed:{e}") from e
    return _enrich(k)
...
async def update_key(key_id: int, req: UpdateKeyReq):
    ...
    updated = state.key_store.update(
        key_id, label=req.label, priority=req.priority, is_active=req.is_active,
        monthly_limit_usd=req.monthly_limit_usd, secret=req.secret,
    )
